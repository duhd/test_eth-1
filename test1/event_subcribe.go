...
func main(){
    	...
    	if err != nil {
			...
			instance, err := contracts.NewCounter(contractAddress, client)
			if err != nil {
				  fmt.Println("Unable to bind to deployed instance of contract")
			}
			fmt.Println("Start listening")

			eventCh := make(chan *contracts.CounterCounterIncreasedEvt,10)
			sub,err := instance.WatchCounterIncreasedEvt(&bind.WatchOpts{Start: nil,  Context: context.Background()},eventCh)
	     ...
	     for {
	         select {
								 case event := <-eventCh:
									  fmt.Println("time:",time.Now(),", owner: ", event.Owner.Hex(), ", Count: ", event.Count)
			        }
	     }
}
