...
func main(){
    	...
    	if err != nil {
			...
			evtIterator,err := instance.FilterCounterIncreasedEvt(&bind.FilterOpts{Start: 0, End: nil,  Context: context.Background()})
			if err != nil {
					fmt.Println("Failed to execute a filter query command", "err", err)
					return
			}
			for  evtIterator.Next() {
				 fmt.Println("owner: ", evtIterator.Event.Owner.Hex(), ", Count: ", evtIterator.Event.Count)
			}
}
